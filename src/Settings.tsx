...
const Settings = ({
	...
	const DNS_PROVIDERS = useMemo<DnsProvider[]>(
		() => [
			{
				id: "system",
				name: t.dnsSystemDefault,
				desc: t.dnsSystemDefaultDesc,
				ip: null,
			},
			{
				id: "cloudflare",
	...
	useEffect(() => {
		if (Object.keys(latencies).length > 0) {
			...
			const otherDns = DNS_PROVIDERS.filter((p) => p.id !== "system").sort(
	...
	const handleFixInternet = async () => {
		...
		try {
			...
			window.dispatchEvent(
				new CustomEvent("bypax-force-disconnect", {
		...
		<div className="v2-settings-overlay">
											...
											>
												...
												<div className="v2-item-text">
													...
													<p
														style={{
															fontSize: "0.75rem",
															color: lang === l.code ? "#93c5fd" : "#71717a",
															marginTop: "2px",
														}}
													>
														{l.code.toUpperCase() === "EN"
															? "EN"
															: l.code.toUpperCase()}
													</p>
												</div>
												<div
													className={`v2-radio ${lang === l.code ? "on" : ""}`}
												>
								...
								<div className="v2-card">
									<div className="v2-item">
										...
										<Toggle
											...
											onChange={(v) => updateConfig("autoConnect", v)}
										/>
									</div>
									...
									<div className="v2-item">
										...
										<Toggle
											...
											onChange={(v) => updateConfig("autoReconnect", v)}
										/>
									</div>
								</div>
							</div>
								...
								<div className="v2-card">
									...
									<div className="v2-item">
										...
										<div className="v2-item-text">
											...
											<p>
												{t.alwaysOnTopDesc ||
													"Pencere her zaman diğer pencerelerin üzerinde kalır"}
											</p>
										</div>
										<Toggle
						...
						>
									...
									{ISP_PROFILES.filter((p) => p.id !== "other").map((isp) => {
										...
										return (
											...
											>
												...
												<div
													onClick={() =>
														setExpandedISP(isExpanded ? null : ispData.id)
													}
													style={{
														padding: "12px 14px",
														display: "flex",
																...
																{!isApplied ? (
																	<button
																		...
																		style={{
																			...
																			alignItems: "center",
																			gap: "6px",
																			boxShadow: `0 4px 12px ${ispData.bg}`,
																	...
																	>
																		<Check size={14} color={ispData.color} />{" "}
																		{t.issAppliedMsg}
																	</div>
																)}
															</div>
									...
									>
										...
										>
											<Activity
												size={20}
												className={
													config.dpiMethod === "0" ? "active-icon" : ""
												}
											/>
										</div>
										<div className="v2-item-text">
											<h3
												style={{
													color: config.dpiMethod === "0" ? "#facc15" : "",
												}}
											>
												{t.modeTurboName}
											</h3>
										...
										<div
											className={`v2-radio ${config.dpiMethod === "0" ? "on" : ""}`}
										>
											{config.dpiMethod === "0" && (
												<div className="v2-radio-dot" />
											)}
										</div>
									</div>
									...
									>
										...
										>
											<Zap
												size={20}
												className={
													config.dpiMethod === "1" ? "active-icon" : ""
												}
											/>
										</div>
										<div className="v2-item-text">
											<h3
												style={{
													color: config.dpiMethod === "1" ? "#4ade80" : "",
												}}
											>
												{t.modeBalancedName}
											</h3>
										...
										<div
											className={`v2-radio ${config.dpiMethod === "1" ? "on" : ""}`}
										>
											{config.dpiMethod === "1" && (
												<div className="v2-radio-dot" />
											)}
									...
									>
										...
										>
											<Shield
												size={20}
												className={
													config.dpiMethod === "2" ? "active-icon" : ""
												}
											/>
										</div>
										<div className="v2-item-text">
											<h3
												style={{
													color: config.dpiMethod === "2" ? "#60a5fa" : "",
												}}
											>
												{t.modeStrongName}
											</h3>
										...
										<div
											className={`v2-radio ${config.dpiMethod === "2" ? "on" : ""}`}
										>
											{config.dpiMethod === "2" && (
												<div className="v2-radio-dot" />
											)}
											...
											>
												...
												{driverInstalled ? (
														...
														<div className="v2-item" style={{ padding: 0 }}>
															...
															<div className="v2-item-text">
																<h3
																	style={{
																		color:
																			config.advancedBypass !== false
																				? "#d8b4fe"
																				: "",
																		fontSize: "0.85rem",
																	}}
																>
																	{t.advancedFeaturesToggle}
																</h3>
																<p style={{ fontSize: "0.7rem" }}>
																	{t.advancedFeaturesToggleDesc}
																</p>
															</div>
															<Toggle
																checked={config.advancedBypass !== false}
																onChange={(v) =>
																	updateConfig("advancedBypass", v)
																}
															/>
														</div>
														{needsRestart && (
													...
													<div style={{ padding: "6px 16px 10px" }}>
														...
														<div
															onClick={() =>
																setShowNpcapDetails(!showNpcapDetails)
															}
															style={{
																display: "inline-flex",
																alignItems: "center",
									...
									{(config.dpiMethod === "1" || config.dpiMethod === "2") && (
										<>
											...
											>
												{CHUNK_SIZES.map((opt) => {
													const fallbackChunk =
														DEFAULT_CHUNKS[config.dpiMethod] || 2;
													const isSelected =
														Number(config.httpsChunkSize || fallbackChunk) ===
														opt.value;
													const accentColor =
														config.dpiMethod === "2" ? "#60a5fa" : "#4ade80";
													const accentBg =
														config.dpiMethod === "2"
							...
							<div className="v2-section">
								...
								<div className="v2-card">
									...
									>
										...
										<Toggle
											checked={config.ipv4Only !== false}
											onChange={(v) => updateConfig("ipv4Only", v)}
										/>
									</div>
									...
									<div className="v2-item" style={{ padding: "1rem" }}>
										...
										<Toggle
											checked={config.enableWinhttp !== false}
											onChange={(v) => updateConfig("enableWinhttp", v)}
										/>
									</div>
								</div>
							</div>
							...
							<div className="v2-section">
								...
								<div className="v2-card">
									...
									<div
										style={{
											opacity: config.selectedDns === "system" ? 0.4 : 1,
											pointerEvents:
												config.selectedDns === "system" ? "none" : "auto",
											transition: "all 0.3s ease",
										}}
									>
										<div className="v2-item">
												...
												{sortedProviders
													...
													.map((p) => {
														...
														return (
															...
															>
																<div
																	className={`v2-radio ${isSelected ? "on" : ""}`}
																>
																	{isSelected && (
																		<div className="v2-radio-dot" />
																	)}
								...
								<div className="v2-card">
									...
									<div
										...
										style={{
											...
											pointerEvents:
												config.notifications !== false ? "auto" : "none",
									...
									>
										...
										<div className="v2-item-text">
											<h3 style={{ fontSize: "0.85rem" }}>
												{t.notifyOnDisconnect}
											</h3>
											<p style={{ fontSize: "0.75rem", color: "#a1a1aa" }}>
												{t.notifyOnDisconnectDesc}
											</p>
										</div>
										<Toggle
							...
							<div className="v2-section">
								...
								>
									<div
										...
										style={{
											cursor: fixStatus === "idle" ? "pointer" : "default",
										}}
									>
										<div
											className="v2-icon"
										...
										>
											<Wrench
												size={20}
												className={
													fixStatus === "fixing" ? "spinning-slow" : ""
												}
											/>
										</div>
										<div className="v2-item-text">
			...
			>
				<button
					...
					style={{ color: activeTab === "general" ? "#fff" : "" }}
				>
					<SettingsIcon
						size={22}
						strokeWidth={activeTab === "general" ? 2.5 : 2}
						style={{ color: activeTab === "general" ? "#60a5fa" : "" }}
					/>
				...
				<button
					...
					style={{ color: activeTab === "network" ? "#fff" : "" }}
				>
					<Globe
						size={22}
						strokeWidth={activeTab === "network" ? 2.5 : 2}
						style={{ color: activeTab === "network" ? "#a855f7" : "" }}
					/>
				...
				<button
					...
					style={{ color: activeTab === "notifications" ? "#fff" : "" }}
				>
					<Bell
						size={22}
						strokeWidth={activeTab === "notifications" ? 2.5 : 2}
						style={{ color: activeTab === "notifications" ? "#10b981" : "" }}
					/>
				...
				<button
					...
					style={{ color: activeTab === "system" ? "#fff" : "" }}
				>
					<Shield
						size={22}
						strokeWidth={activeTab === "system" ? 2.5 : 2}
						style={{ color: activeTab === "system" ? "#eab308" : "" }}
					/>
