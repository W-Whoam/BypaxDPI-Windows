...
function App() {
	...
	const startEngine = async (_ignoredPort: number, portRetryCount = 0) => {
		...
		try {
			...
			const userChunk = [1, 2, 4, 8, 16].includes(
			...
			if (dpiMethod === "2") {
				...
				if (hasDriver && advancedBypass) {
					...
					addLog(
						t.logStrongFake || "🚀 Güçlü Mod: Fake Paket (3) aktif.",
						"success",
					);
				} else {
					...
					if (!hasDriver) {
						addLog(
							t.logStrongNoDriver ||
								"⚠️ Güçlü Mod: Sürücü yok, sadece Chunk-1 aktif.",
							"warn",
						);
					} else {
						addLog(
							t.logStrongChunkOnly || "🛡️ Güçlü Mod: Chunk-1 aktif.",
							"info",
						);
					}
				}
				...
				if (!connectionConfirmed && isReady) {
					...
					try {
						...
						if (configRef.current.enableWinhttp !== false) {
							addLog(t.logWinHttpEnabled, "warn", {
								i18nKey: "logWinHttpEnabled",
							});
						}
					} catch (err) {
			...
			command.on("close", (data) => {
				if (!isRetrying.current) {
					...
					const isStrongWithFake =
						configRef.current.dpiMethod === "2" &&
						configRef.current.advancedBypass !== false;
	...
	const toggleConnection = async () => {
		...
		if (isProcessing || isRestartingDpi.current || isRestartingLan.current)
			return;
	...
	useEffect(() => {
		...
		config.dpiMethod,
		config.httpsChunkSize,
		config.selectedDns,
		config.dnsMode,
		config.enableWinhttp,
		config.ipv4Only,
		isConnected,
	]);
		...
		<div className="app-container fade-in">
							...
							<p style={{ color: "#a1a1aa", fontSize: "0.95rem" }}>
								<AnimatePresence mode="wait">
									...
									>
										{closingStep === 0
											? (
													t.logShutdownStarting ||
													"Güvenli bağlantı sonlandırılıyor"
												).replace(/\.+$/, "")
						...
						>
							...
							>
								<button
									...
									onClick={() => openUrl(URLS.tutorialHowItWorks)}
								>
									<HelpCircle size={18} />
									{t.adminHowItWorks}
								</button>
					...
					>
						<div
							style={{
								...
								height: "400px",
								background:
									"radial-gradient(circle, rgba(59, 130, 246, 0.08) 0%, rgba(0,0,0,0) 60%)",
								pointerEvents: "none",
								zIndex: 0,
							}}
						/>
									...
									return (
										...
										>
											...
											<div style={{ flex: 1, textAlign: "left" }}>
												<div
													style={{
														color: isSelected ? isp.color : "#f8fafc",
														fontWeight: 600,
														fontSize: "0.9rem",
													}}
												>
			...
			<div className="action-area">
				...
				<button
					...
					disabled={
						isProcessing || isRestartingDpi.current || isRestartingLan.current
					}
				>
										...
										>
											...
											>
												<strong style={{ color: "#ef4444" }}>
													{t.modalPacWarningTitle}
												</strong>{" "}
												{t.modalPacWarningDesc}
											</div>
										</div>
								...
								{connectionModalTab === "manual" && (
									<>
										...
										>
											...
											>
												<strong style={{ color: "#ef4444" }}>
													{t.modalManualWarningTitle}
												</strong>{" "}
												{t.modalManualWarningDesc}
											</div>
										</div>
										<p
											style={{
										...
										>
											<div>
												...
												>
													<span
														style={{
															color:
																copiedField === "host" ? "#4ade80" : undefined,
														}}
													>
														{lanIp}
													</span>
											...
											<div>
												...
												<div
													className="code-box"
													onClick={() =>
														handleCopyField(currentPort.toString(), "port")
													}
													title="Kopyala"
													style={{
														transition: "all 0.2s",
												...
												>
													<span
														style={{
															color:
																copiedField === "port" ? "#4ade80" : undefined,
														}}
													>
														{currentPort}
													</span>
